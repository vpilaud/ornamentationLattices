...
\label{prop:acyclicReorientationLatticesJoinMeet}
...
\[
\rev(R_1 \join R_2) = \tc \big( \rev(R_1) \cup \rev(R_2) \big)
\quad\text{and}\quad
E \ssm \rev(R_1 \meet R_2) = \tc \big( E \ssm (\rev(R_1) \cap \rev(R_2)) \big).
\]
\end{proposition}
...
since their fibers under~$R \mapsto \orn{R}$ are all singletons, containing the cyclic reorientations of~$\tc(D)$ given by
\[
...
In this section, we focus on the specific situation when the ground directed graph~$D$ is a directed tree~$T$ (but not necessarily a rooted tree), as for example in  in \cref{fig:ornamentationsNI,fig:ornamentationsAY,fig:ornamentationsX}.
...
\begin{lemma}
For a directed tree~$T$ and an acyclic reorientation~$R$ of~$\tc(T)$, we have~$u \in \orn{R}(v)$ if and only if~$u' \in \rev(R)$ for all~$u'$ along the path from~$u$ to~$v$ in~$T$.
\end{lemma}
...
Hence, any induced oriented cycle in~$\reori{O}$ is an induced cycle in~$\tc(T)$.
\end{lemma}
...
\begin{lemma}
\label{lem:X}
For a directed tree~$T$, any induced cycle in~$\tc(T)$ is of the form~$(a,c,b,d)$ where~$a,b,c,d$ are starred.
\end{lemma}

\begin{proof}
Let~$C$ be an induced cycle in~$\tc(T)$.
As~$\tc(T)$ is transitive, $C$ is alternating (meaning taking alternatively forward and backward arrows).
We can thus write~$C = (d_1, u_1, d_2, u_2, \dots, d_k, u_k)$ where~$(d_i, u_i)$ and~$(u_i, d_{i+1})$ are in~$\tc(T)$ for all~$i \in [k]$.
...
We work by induction on the length of the smallest induced cycle.
\end{proof}

\begin{remark}
\label{rem:MacNeille}
Note that~\cref{thm:MacNeille} fails beyond directed trees.
For instance, for the directed graph of  \cref{fig:ornamentationsD}, the acyclic ornamentation poset is not a lattice, but adding the two cyclic ornamentations
\[
...
\section{Starred-free trees and rooted trees}
\label{sec:rootedTrees}

In this section, we discuss the specific situation when the ground directed graph~$D$ is a starred-free tree~$T$, defined as follows

\begin{lemma}
\label{lem:starredFreeTrees}
The following conditions are equivalent for a directed tree~$T$:
\begin{enumerate}[(i)]
\item there is no~$u,v \in T$ such that~$u$ has at least two incoming edges in~$T$, $v$ has at least two outgoing edges in~$T$, and $T$ has a directed path from~$u$ to~$v$,
\item there is no $a,b,c,d,e \in V$ such that~$(a,c)$, $(b,c)$, $(c,d)$, and~$(c,e)$ are all in~$\tc(T)$,
\item $\tc(T)$ contains no induced cycle.
\end{enumerate}
\end{lemma}
...
\begin{definition}
We say that a directed free~$T$ is \defn{starred-free} if it satisfies the equivalent conditions of \cref{lem:starredFreeTrees}.
Note that rooted trees (with increasing edges all oriented toward the root) are starred-free.
\end{definition}

\begin{proposition}
\label{prop:rootedTrees1}
For any ornamentation~$O$ of a starred-tree tree~$T$, the reorientation~$\reori{O}$ of~$\tc(T)$ and the sourcing~$\sour{O}$ of~$\PP(T)$ are both acyclic.
...
\vincent{Need to be rewritten for starred-free.}
%For Part (1),
...
For a starred-free tree~$T$, the acyclic reorientation poset~$\AReori(\tc(T))$, the acyclic sourcing poset~$\ASour(\PP(T))$, and the acyclic ornamentation poset~$\AOrn(T)$ are all lattices.
\end{proposition}
...
We conclude by~\cref{prop:acyclicReorientationLatticesJoinMeet} that~$\AReori(\tc(T))$ is a lattice.
...
For a starred-free tree~$T$, the map $R \mapsto \aorn{R}$ is a surjective lattice map, hence the lattices~$\ASour(\PP(T)) \simeq \AOrn(T) = \Orn(T)$ are lattice quotients~of~$\AReori(\tc(T))$.
\end{proposition}

\begin{proof}
The map is surjective by \cref{def:AOrn}, thus we only need to show that ${\orn{R_1 \meet R_2} = \orn{R_1} \meet \orn{R_2}}$ and~$\orn{R_1 \join R_2} = \orn{R_1} \join \orn{R_2}$ for any two acyclic reorientations~$R_1$ and~$R_2$ of $\tc(T)$.
Remember the formulas for~$\rev(R_1 \meet R_2)$ and~$\rev(R_1 \join R_2)$ from~\cref{prop:acyclicReorientationLatticesJoinMeet}, and let us define by~$O_\meet \eqdef \orn{R_1} \meet \orn{R_2}$ and~$O_\join \eqdef \orn{R_1} \join \orn{R_2}$.

%Observe that~$u \in O_\join(v)$ is equivalent by \cref{thm:OrnMeetJoin} to the existence of a path~$u = w_0, \dots, w_p = v$ such that~$w_{i-1} \in $

\vincent{todo}
\end{proof}

\begin{corollary}
If~$T$ is a starred-free tree, then the Hasse diagram of the ornamentation lattice~$\Orn(T)$ is isomorphic to the graph of the path hypergraphic polytope~$\simplex_{\PP(D)} \eqdef \sum_{u \le_T v} \simplex_{[u,v]_T}$ oriented in the direction~$\omega$.
\end{corollary}
...
We can reformulate \cref{prop:rootedTrees} using the language of~\cite{Pilaud-acyclicReorientationLattices}: if~$T$ is a starred-free tree, then
\begin{enumerate}
...
We denote by~$B_{m,n}$ the number of (acyclic) ornamentations of the $(m,n)$-broom, by~$B(x,y) \eqdef \sum_{m \ge 0, n \ge 0} B_{m,n} \, x^m/m! \, y^n$ their generating function (exponential in~$x$ and ordinary in~$y$), and by~$B_m(y) \eqdef m! \, [x^m] \, B(x,y) = \sum_{n \ge 0} B_{m,n} \, y^n$ the coefficient of~$x^m$ in~$B(x,y)$.
...
\[
B(x,y) = \frac{e^x}{1-e^x \cdot y \cdot C(y)}
\]
