...
For~$u,v \in V$, we write~$u \le_T v$ if the unique path between~$u$ and~$v$ in~$T$ is a directed path from~$u$ to~$v$.
...
Write~$C = (d_1, u_1, \dots, d_k, u_k)$ where~$(d_i, u_i)$ and~$(d_{i+1},u_i)$ are in~$\tc(T)$ for all~$i \in [k]$.
...
Write~$C = (d_1, u_1, \dots, d_k, u_k)$ where~$(d_i, u_i)$ and~$(d_{i+1}, u_i)$ are in~$\tc(T)$ for all~$i \in [k]$.
As~$C$ is induced, $d_i$ and~$d_j$ (resp.~$u_i$ and~$u_j$) are incomparable for any~$i \ne j$.
...
Let~$w$ denote the last vertex before~$u_k$ along the path from~$d_1$ to~$u_k$.
If~$w$ does not belong to the path from~$d_k$ to~$u_k$, then the cycle~$C$ must pass again through~$u_k$, contradicting inducedness.
\Antoine{I think I don't understand this.
If $T$ is two X's of \cref{fig:ornamentationsX}, say X and X', but with 5 and 4' glued together, then taking $C=(1,5,1',5',2',5,2)$, $w=3$ does not work. I proposed another proof.}

Hence, we obtain a new induced alternating cycle by replacing~$u_k$ by~$w$ in~$C$.
...
\antoine{I propose: We take $C$ to be minimal (in the sense that it is not two cycles glued together).
	Let~$w$ denote the last vertex before~$u_k$ along the path from~$d_1$ to~$u_k$.
	If~$w$ does not belong to the path from~$d_k$ to~$u_k$, then the cycle~$C$ must pass again through~$u_k$,
	\red{contradicting minimality of $C$}.
	Now, there exists $i$ such that $d_j<w<u_i\neq u_k$ where $j\in\{i,i-1\}$. We have $a=d_1$, $b=d_j$, $c=w$, $d=u_k$ and $e=u_i$.}
...
The ornamentation~$O = \orn{R}$ of~$T$ with~$O(d) = \set{w}{a \le_T w \le_T d}$, $O(e) = \set{w}{b \le_T w \le_T e}$, and~$O(w) = \{w\}$ for all~$w \in V \ssm \{d,e\}$ is cyclic.
...
We say that an intreeval hypergraph~$\II$ of an increasing tree~$T$ is \defn{path intersection closed} if for all~$I,J\in \II$ with~$|I\cap J|> 1$ such that $\min(I) <_T \min(I\cap J)$ and  $\max(J) >_T \max(I\cap J)$, there exists~$K \in \II$ such that 
\jose{$I \cap J \subseteq K \subseteq [\min(J) , \max(I)]_T$.\sout{$\min(J) \le_T \min(K) \le_T \min(I\cap J)$ and ${\max(I\cap J) \le_T \max(K) \le_T \max(I)}$.}}\Jose{I was getting confused with all the inequalities. Would this be easier to parse for the reader?}
\end{definition}
...
We say that~$\II$ is \defn{star sparse} if~$X(T,u,v,\II)$ is acyclic for all~$u,v$ connected by a directed path. \Antoine{Would it point out more the link with the induced cycles of last section to denote the vertices by $d_1,\cdots d_k$ and $u_1\cdots u_k$ instead of $u_1\cdots u_k$ and $v_1\cdots v_k$?}
\end{definition}
...
Assume by means of contradiction that~$\ASour(\II)$ is a lattice but that there are~$I,J\in \II$ with~$|I\cap J|> 1$ such that $\min(I) <_T \min(I\cap J)$ and  $\max(J) >_T \max(I\cap J)$, and there is no~$K \in \II$ such that $\min(J) \le_T \min(K) \le_T \min(I\cap J)$ and ${\max(I\cap J) \le_T \max(K) \le_T \max(I)}$.
\Antoine{Should we replace this by Jose's description?}
