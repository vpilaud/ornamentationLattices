...
We now assume that~$V$ is totally ordered, for instance that~$V = [n]$ with the natural order.
...
In fact, it also follows from~\cref{lem:Reori2Orn2} since the map~$S \mapsto R$ where~$\rev(R) = \rev(S) \cap \tc(D)$ is order preserving.
...
\begin{lemma}
The map~$O \mapsto S_O$ is a section of the map~$S \mapsto O_{\!S}$, which is therefore a surjection from the sourcing lattice~$\Sour(\PP(D))$ to the ornamentation lattice~$\Orn(D)$.
\end{lemma}
...
To sum up, the map~$S \mapsto O_{\!S}$ is an order-preserving surjection from the sourcing lattice~$\Sour(\PP(D))$ to the ornamentation lattice~$\Orn(D)$.
