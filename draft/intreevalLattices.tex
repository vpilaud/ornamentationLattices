...
\subsection{Ornamentation lattices}
...
.
\]
We will see in \cref{lem:Orn2ReoriDT} that $\reori{O}$ is both transitively closed and coclosed when~$D$ is a directed tree.
\end{remark}
...
We will see in \cref{prop:ReoriTC2OrnIntervalTree} that it does if~$D$ is a directed tree.
\end{remark}
...
\begin{remark}
We will see in \cref{subsec:rootedTrees} that all ornamentations of a rooted tree~$T$ (with increasing edges all oriented toward the root) are acyclic.
...
\vincent{What about the sourcing~$\sour{O}$?}
\end{proof}
...
\begin{proof}
\vincent{Give the other argument here...}
Recall that if
\[
