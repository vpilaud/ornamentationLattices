...
When~$T$ is a path, these conditions specialize to that of~\cite{BergeronPilaud} as in this case, path intersection closedness boils down to intersection closedness, and star sparsity always~holds. 
...
In particular, the acyclic sourcing poset of any intersection closed intreeval hypergraph of a rooted tree is a lattice.
...
See \eg \cref{fig:ornamentationsD}.
\end{proof}
...
We denote by~$\rev(S)$ the set of pairs~$(u,v) \in V^2$ such that there is a directed path~$P$ in~$D$ from~$u$ to~$v$ with~$S(P) = v$.
...
For any~$(u,v) \in \rev(S_1)$, there is a path~$P$ from~$u$ to~$v$ with~$S_1(P) = v$.
...
\begin{lemma}
\label{lem:Orn2Sour2}
The following are equivalent for any ornamentation~$O$ of~$D$ and vertices~$u,v \in V^2$:
\begin{enumerate}[(i)]
...
For each $1 \le i \le p$, as~$(w_{i-1}, w_i) \in \rev(\sour{O})$, there is a path~$P_i$ from~$w_{i-1}$ to~$w_i$ with~$\sour{O}(P_i) = w_i$ by \cref{def:Sour2Reori}, hence~$w_{i-1} \in O(w_i)$ by \cref{def:Orn2Sour}.
Since~$O$ is an ornamentation, an immediate induction shows that~$w_i \in O(v)$ for all~$0 \le i \le p$.
...
Then there is a path~$P$ from~$u$ to~$v$ in~$O(v)$.
Since~$u \in O(v)$, we have~$\sour{O}(P) = v$ by \cref{def:Orn2Sour}.
...
For instance, on the graph~\includegraphics[scale=.8,valign=c]{graphI}, for the sourcing~$S$ given by~$S(\{1,2\}) = 1$, $S(\{2,3\}) = 2$ and~$S(\{1,2,3\}) = 3$, we have $\orn{S} = \raisebox{-.2cm}{\includegraphics[scale=.8]{ornamentationI1}}$ so that
\[
...
If~$S$ is an acyclic sourcing of~$\PP(D)$, then~$\rev(S)$ is transitive.
\end{lemma}
...
Their concatenation~$PQ$ is a path from~$u$ to~$w$ with~$S(PQ) = w$ by acyclicity of~$S$ (indeed, $PQ$ and~$P$ for a cycle in~$S$ if~$S(PQ) \in P \ssm \{v\}$, and $PQ$ and~$Q$ for a cycle in~$S$ if~$S(PQ) \in Q \ssm \{w\}$).
Hence~$(u,w) \in \rev(S)$.
\end{proof}
...
Then~$u < v$ and there is a path~$P \in \PP(D)$ from~$u$ to~$v$ such that~$\asour{R}(P) = v$.
...
Let~$u = w_0, \dots, w_k = v$ denote the vertices of this path.
...
For any acyclic reorientation~$R$ of~$\tc(D)$ (resp.~acyclic sourcing~$S$ of~$\PP(D)$) and any~$u,v \in [n]$, we have~$u \in \orn{R}(v)$ (resp.~$\orn{S}(v)$) if and only if~$(u,v) \in \rev(R)$ (resp.~$\rev(S)$).
\end{lemma}
...
We have seen in \cref{lem:Orn2Sour3} that the map~$S \mapsto \orn{S}$ is surjective on all ornamentations.
