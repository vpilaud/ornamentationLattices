...
\urladdr{https://felixgelinas.github.io/}
...
Tamari lattices and associahedra are classical objects of algebraic and geometric combinatorics with various applications in topology, algebra, statistical physics, etc.
...
The objective of this paper is to study the connections between natural extensions of these three perspectives, already considered separately in the literature.
...
See~\cite{Pilaud-acyclicReorientationLattices} for a characterization of the directed graphs~$E$ for which~$\AReori(E)$ is a lattice (or even a semidistributive lattice).
...
The latter is obtained by deleting inequalities in the facet description of the graphical zonotope of~$E$.
...
The ornamentation poset~$\Orn(D)$ is always a lattice, but the acyclic ornamentation poset~$\AOrn(D)$ is not always a lattice.
...
\begin{figure}
	\centerline{\input{figures/diagram.tex}}
...
It is a priori not required that~$D$ is acyclic, although this is the case that interests us most in this paper.
...
The surjection~$R \mapsto \orn{R}$ preserves neither the meet nor the join semilattice structure, even when~$D$ is a path.
For instance,
...
\]
are cyclic biclosed reorientations of $\tc(\Xgraph)$, where $\Xgraph$ is the graph in \cref{fig:ornamentationsX}.
...
We consider the following maps on hypergraphs.
...
Note that these maps are usually called \emph{orientations of hypergraphs}, we have preferred the term \emph{sourcings of hypergraphs} to avoid confusion with the reorientations of graphs.
 
We now assume that~$D$ is an increasing graph on~$V = [n]$, meaning that~$u < v$ for any edge~$(u,v)$ of~$D$. In particular, $D$ is acyclic.
...
Acyclic reorientations of directed acyclic graphs are a classical subject, see for instance~\cite{Greene,GreeneZaslavsky,Pilaud-acyclicReorientationLattices} and \cref{fig:acyclicReorientationsAY,fig:acyclicReorientationsD,fig:acyclicReorientationsKT} for illustrations.
...
The \defn{graphical zonotope} of~$E$ is the Minkowski sum~$\simplex_E \eqdef \sum_{(u,v) \in E} \simplex_{uv}$, where~$\simplex_{uv}$ is the segment with endpoints~$\b{e}_u$ and~$\b{e}_v$.
The face lattice of~$\simplex_E$ is described by \emph{ordered partitions} of~$E$.
...
For a permutation~$\pi$ of~$[n]$, we denote by~$\areori{\pi}$ the reorientation of~$\tc(D)$ where $(u,v) \in \tc(D)$ is reversed in~$\areori{\pi}$ if~$\pi^{-1}(u) > \pi^{-1}(v)$.
\end{definition}
...
We will further study this question for subhypergraphs of the path hypergraph of increasing trees in \cref{sec:intreevalHypergraphicPosets}, which generalizes \cite{BergeronPilaud}.
\end{remark}
...
For instance, consider the directed graph~$D$, the two reorientations~$R_1$ and~$R_2$ of~$\tc(D)$, and the two sourcings~$S_1$ and~$S_2$ of~$\PP(D)$ given by
\begin{gather*}
D = \! \includegraphics[scale=.8,valign=c]{graphR}
\qquad\qquad
...
S_2(13) = 3, \;\; S_2(134) = 3, \;\; S_2(15) = 5, \;\; S_2(24) = 4, \;\; S_2(25) = 5, \;\; S_2(34) = 3.
\end{gather*}
...
Namely, using that the hypergraphic polytope of $\PP(D)$ is a deformation of the graphical zonotope of $\tc(D)$.
...
We conclude that~$u \in \orn{\asour{R}}(v)$ by maximality of~$\orn{\asour{R}}(v)$, hence that~$\orn{R}(v) \subseteq \orn{\asour{R}}(v)$.
...
Hence,~$u \in P \subseteq \aorn{S_1}(v) \subseteq \aorn{S_2}(v)$.
...
Hence, $u \in \orn{R}(v)$ if and only if~$(u',v) \in \rev(R)$ for all~$u'$ along the path from~$u$ to~$v$ in~$T$.
\end{proof}
...
The irredundant join representations in~$L$ are antichains of~$L$, and are ordered by containment of the lower sets they generate (\ie~$J \le J'$ if and only if for any~$y \in J$ there exists~$y' \in J'$ such that~$y \le y'$ in~$L$).
...
We first briefly recall the definition of the \emph{MacNeille completion} of a poset.
...
The \defn{MacNeille completion}~$C(P)$ of~$P$ is the smallest completion of~$P$, meaning that for any completion~$L$ of~$P$, there exists an order-embedding from~$C(P)$ to~$L$.
\end{definition}
...
The reorientation~$R_i$ clearly covers~$R$, and we claim that is still biclosed.
Indeed,
\begin{itemize}
...
\end{itemize}
Another interesting case is the directed graph~$\Kgraph$ of \cref{fig:ornamentationsKT,fig:reorientationsKNT,fig:acyclicReorientationsKT}, for which the acyclic ornamentations already form lattice, even if there exists a cyclic ornamentation.
...
Let~$w$ denote the last vertex before~$v_k$ along the path from~$u_1$ to~$v_k$ in $T$.
...
We can reformulate \cref{prop:AReoriLatticeUT,prop:AOrnLatticeUT,prop:AReori2AOrnLatticeMapUT} in the language of~\cite{Pilaud-acyclicReorientationLattices}: for an unstarred~tree~$T$,
\begin{enumerate}
...
For instance, the fiber of the acyclic sourcing identified with the ornamentation
\[
...
\begin{definition}
A \defn{Dyck path} of semilength~$n$ is a path with up steps~$(1,1)$ and down steps~$(1,-1)$ starting at~$(0,0)$, ending at~$(2n,0)$, and never passing strictly below the horizontal axis.
...
In this section, we characterize the subhypergraphs~$\II$ of the path interval~$\PP(T)$ of an increasing tree~$T$ whose acyclic sourcing poset~$\ASour(\II)$ is a lattice, extending results of N.~Bergeron and V.~Pilaud for interval hypergraphs~\cite{BergeronPilaud}.
...
We say that an intreeval hypergraph~$\II$ of an increasing tree~$T$ is \defn{path intersection closed} if for all~$I,J\in \II$ with~$|I\cap J|> 1$ such that $\min(I) <_T \min(I\cap J)$ and  $\max(J) >_T \max(I\cap J)$, there exists~$K \in \II$ such that~$I \cap J \subseteq K \subseteq [\min(J) , \max(I)]_T$. %$\min(J) \le_T \min(K) \le_T \min(I\cap J)$ and ${\max(I\cap J) \le_T \max(K) \le_T \max(I)}$.
\end{definition}
...
\item the next four are star sparse but not path intersection closed,
...
As~$\ASour(\II)$ is a lattice, we have an acyclic sourcing~$S_G$ of~$\II$ such that
\[
S_A \le S_G
\qquad
S_B \le S_G
...
\begin{enumerate}[(i)]
\item if~$\min(I) \le_T g <_T b$, then~$S_G(I) = g < b = S_A(I)$, contradicting that~$S_A < S_G$,
\item if~$\min(J) \le_T g <_T c$, then~$S_G(J) = g < c = S_B(J)$, contradicting that~$S_B < S_G$,
\item if~$c <_T g \le \max(J)$, then~$S_G(J) = g > c = S_D(J)$, contradicting that~$S_G < S_D$,
\item if~$b <_T g \le_T \max(I)$, then~$S_G(I) = g > b = S_C(I)$, contradicting that~$S_G < S_C$.
\end{enumerate}
Note that the case $g = b$ (resp.~$g = c$) is covered by Case~(ii) (resp.~(iii)), hence these four cases cover all possibilities.
...
\begin{array}{c|cccccc}
	Z & A & B & C & D & E & F \\
	\hline
	\pi_Z & 314625 & 146325 & 631425 & 463125 & 164325 & 163425 \\
	S_Z(\blue{1345}) & 3 & 1 & 3 & 4 & 1 & 1 \\
	S_Z(\red{2346}) & 3 & 4 & 6 & 4 & 6 & 6
\end{array}
\]
See \cref{fig:pathIntersectionClosedStarSparse}\,(left).
The reader can check that~$S_A < S_C$, $S_A < S_D$, $S_B < S_E = S_F < S_C$, and~$S_B < S_D$.
%
\begin{figure}
	\centerline{
		\input{figures/pathIntersectionClosed}
		\hspace{1.5cm}
		\input{figures/starSparse}
	}
	\caption{The permutations and sourcings of \cref{exm:pathIntersectionClosed} (left) and of \cref{exm:pathStarSparse} (right), illustrating the proofs of \cref{lem:pathIntersectionClosed,lem:pathStarSparse}. The black arrows indicate relations in weak order. The source of each hyperedge is colored in the color of the hyperedge.}
	\label{fig:pathIntersectionClosedStarSparse}
\end{figure}
\end{example}
...
Let~$W$ be a word formed by the other vertices~$[n] \ssm \{u_1, v_1, \dots, u_p, v_p\}$ in any order.
...
Then any minimal cycle in any sourcing of~$\II$ has length~$2$.
\end{proposition}
...
\begin{figure}[b]
	...
	\coordinate (F) at (2,0.3);
	\coordinate (H) at (3,1.5);
	\coordinate (G) at (3,1.2);

	\node[brown] at (-.55,1.5) {\scriptsize $w_{2j-2}$};
	\node at (-.55,1.2) {\scriptsize $w'_{2j-2}$};
	\node[red] at (0.45,0.0) {\scriptsize $w_{2j-1}$};
	\node at (0.45,0.3) {\scriptsize $w'_{2j-1}$};
	\node[orange] at (1.3,1.5) {\scriptsize $w_{2j}$};
	\node at (1.3,1.2) {\scriptsize $w'_{2j}$};
	\node[green] at (2.45,0.0) {\scriptsize $w_{2j+1}$};
	\node at (2.45,0.3) {\scriptsize $w'_{2j+1}$};
	\node[blue] at (3.45,1.5) {\scriptsize $w_{2j+2}$};
	\node at (3.45,1.2) {\scriptsize $w'_{2j+2}$};

	\node[red] at (-.6,.6) {\scriptsize $I_{2j-1}$};
	\node[orange] at (.7,.6) {\scriptsize $I_{2j}$};
	\node[green] at (1.4,.6) {\scriptsize $I_{2j+1}$};
	\node[blue] at (2.8,.6) {\scriptsize $I_{2j+2}$};
	
	\node at (-2,.75) {$\cdots$};
	\draw[brown] (-1.5,.75) to [out=10,in=270] ([xshift=-.2] Y) -- ([xshift=-.2] Z);
	\draw[red] ([xshift=-.2] A.center) -- ([xshift=-.2] B.center) to [out=90,in=270] ([xshift=.2] Y) -- ([xshift=.2] Z);
	\draw[orange] ([xshift=.2] A) -- ([xshift=.2] B) to [out=90,in=270] ([xshift=-.2] C) -- ([xshift=-.2] D);
	\draw[green] ([xshift=-.2] E) -- ([xshift=-.2] F) to [out=90,in=270] ([xshift=.2] C) -- ([xshift=.2] D);
	\draw[blue] ([xshift=.2] E) -- ([xshift=.2] F) to [out=90,in=270] ([xshift=-.2] G) -- ([xshift=-.2] H);
	\draw[purple] (3.5,.75) to [out=170,in=270] ([xshift=.2] G) -- ([xshift=.2] H);
	\node at (4,.75) {$\cdots$};

	\node[fill,circle,inner sep=1pt] at (Z) {};
	\node[fill,circle,inner sep=1pt] at (Y) {};
	\node[fill,circle,inner sep=1pt] at (A) {};
	\node[fill,circle,inner sep=1pt] at (B) {};
	\node[fill,circle,inner sep=1pt] at (D) {};
	\node[fill,circle,inner sep=1pt] at (C) {};
	\node[fill,circle,inner sep=1pt] at (E) {};
	\node[fill,circle,inner sep=1pt] at (F) {};
	\node[fill,circle,inner sep=1pt] at (H) {};
	\node[fill,circle,inner sep=1pt] at (G) {};
...
We are grateful to the anonymous referee for a careful reading of the manuscript and for valuable corrections and suggestions.
