...
To fix a convention, let us denote by~$1, \dots, 2n-1$ the tooth and by~$2, \dots, 2n$ the handle of the $n$-comb, so that the parent of node~$i$ is $i+1$ if~$i$ is odd and~$i+n$ if~$i$ is even (except the node~$2n$ which is the root).
See \cref{fig:comb}.
