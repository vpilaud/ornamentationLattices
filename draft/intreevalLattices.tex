...
\newcommand{\Kgraph}{\boldsymbol{\vartriangleleft}} % K graph
...
In contrast, not all reorientations of~$\tc(D)$ can be projected to a sourcing of~$\PP(D)$, because reorientations might have cycles inside paths of~$D$. \felixantoine{What does cycles inside paths means?}
...
\FelixAntoine[inline]{Since $O \mapsto \asour{O}$ is not the restriction of $O \mapsto \sour{O}$, we cannot use that the second is order-preserving to deduce that the first is. We modified the proof accordingly.}

\begin{proof}
The map~$\ASour(\PP(D)) \to \AOrn(D) : S \mapsto \aorn{S}$ is injective by \cref{lem:ASour2AOrn1} and surjective by \cref{def:AOrn}.
Moreover, it is the restriction of the order-preserving map $S \to \orn{S}$ (\cref{lem:Sour2Orn3}), and therefore it is itself order-preserving.
We are only left to show that $S \mapsto \aorn{S}$ is order-reflecting.

Let $S_1,S_2$ be two acyclic sourcings of $\PP(D)$ such that $\aorn{S_1} \leq \aorn{S_2}$.
Suppose that $S_1 \not\leq S_2$ and let $P \in \PP(D)$ such that $u := S_2(P) < S_1(P) =: v$.
...
so $u \in P \subseteq \aorn{S_1}(P) \subseteq \aorn{S_2}(P)$.
...
Two ornamentations~$O_1$ and~$O_2$ of a directed graph~$D$ form a cover relation~$O_1 \lessdot O_2$ if and only if there are~$u, v \in V$ such that~$u \notin O_1(v)$ and $O_2(v) = O_1(u) \cup O_1(v)$ while~$O_1(w) = O_2(w)$ for all~${w \in V \ssm \{v\}}$.
Moreover, $u$ and~$v$ are uniquely determined by~$O_1$ and~$O_2$.
\end{lemma}\Antoine{No need the graph to be a tree. Should we put the result earlier in the article?}

\begin{proof}
Assume first that~$O_1$ and~$O_2$ are two ornamentations of~$D$ such that there are~$u, v \in V$ with~$u \notin O_1(v)$ and~$O_2(v) = O_1(u) \cup O_1(v)$ while~$O_1(w) = O_2(w)$ for all~${w \in V \ssm \{v\}}$.
...
If~$O_1 \ne O$, then $O(v) \ssm O_1(v)\neq \emptyset$ and let~$y$ be a maximal element in $O(v) \ssm O_1(v)$.
...
Moreover, $O_1 \le O < O_2$.
Hence, $O_1 = O$.
Consider now~$u$ maximal in~$D$ such that~$u \in O_2(v) \ssm O_1(v)$.
...
Observe now that~$O_1 \le $\sout{$O$} $\felix{M_P}$ (as~$u \notin O_1(v)$, we have~$\lessin{T}{u} \cap O_1(w) = \varnothing$ for all~$u <_T w \le_T v$) but~$O_2 \not\le$ \sout{$O$} $\felix{M_P}$ (because~$u \in O_2(v)$ while~$u \notin M_P(v)$).
