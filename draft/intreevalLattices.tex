...
The question to characterize the hypergraphs~$\HH$ whose acyclic sourcing poset~$\ASour(\HH)$ is a lattice has been studied for specific families of hypergraphs, notably for graphs~\cite{Pilaud-acyclicReorientationLattices}, for interval hypergraphs~\cite{BergeronPilaud}, and (partially for) graph associahedra~\cite{BarnardMcConville}.
\Vincent{I would still use plural here, but I am not sure...}
...
The face lattice of~$\simplex_\HH$ was described combinatorially in terms of acyclic orientations
\Jose{should we say "sourcings" here?}
\Vincent{I would keep it as it is here.}
of~$\HH$ in~\cite{BenedettiBergeronMachacek}.
...
\Jose{We used $\binom{[n]}{2}$ for the complete graph before (though it was in the context of hypergraphs), should we use the same notation in both places?}
\Vincent{I would keep it as it is here.}
\begin{itemize}
...
\Jose{I added this sentence to have a color-independent explanation of the pictures}
\Vincent{Good point. I changed it a little bit.}
...
For~$u,v \in V$, we write~$u \le_T v$ if there is a (unique) directed path from~$u$ to~$v$ in~$T$. %, and we write~$[u,v]_T$ for this path.
\Vincent{Or we could say: if the path from~$u$ to~$v$ in~$T$ is oriented from~$u$ to~$v$...}
We denote by~$\lessin{T}{v} \eqdef \set{u \in V}{u \le_T v}$ the set of vertices which admit a directed path towards~$v$ in~$T$.
...
\begin{proof}
\jose{\sout{In a directed tree, the ornaments are precisely the directed paths, and the intersection of two directed paths is a directed path.}}\Jose{The first claim is not true if there is a vertex with in-degree at least $2$}
\Vincent{Good point. It was completely wrong.}
...
For a directed tree~$T$, an acyclic reorientation~$R$ of~$\tc(T)$ and vertices $u \le_T v$, we have~$u \in \orn{R}(v)$ if and only if~$(u',v) \in \rev(R)$ for all~$u \le_T u' \le_T v$.
\end{lemma}

\begin{proof}
Since~$R$ is acyclic, for any $u \le_T v$, there is a directed path from~$u$ to~$v$ in~$\rev(R)$ if and only if~$(u,v) \in \rev(R)$.
...
\Jose{The first time I read this sentence it seemed to me that we were assuming that $u'$ is also in the path, so I added the extra clarification.}
\Vincent{Yes, it helps. I was not sure if it was needed, but it is better with it.}
Otherwise, $M_P(u') \subseteq \lessin{T}{u'} \subseteq \lessin{T}{v'} = M_P(v')$.
\end{proof}
...
Any cycle in~$\reori{M_P}$ should have an edge of $\tc(T) \ssm \rev(\reori{M_P})$, hence a vertex in~$\lessin{T}{u}$ and a vertex in~$P \ssm \{u\}$.
This is impossible as there is no edge of~$\reori{M_P}$ leaving~$P \ssm \{u\}$ \Felix{and arriving in $\lessin{T}{u}$}\Vincent{I stand with my version.} \Jose{I agree with Félix: those $w \le_T v$ that are not in the path $P$ nor below $u$ correspond to arrows $(w,v) \in \rev(\reori{M_P})$}.
\end{proof}
...
\item there are distinct $a,b,c,d,e \in V$ such that~$(a,c)$, $(b,c)$, $(c,d)$, and~$(c,e)$ are all in~$\tc(T)$, \Felix{We need to ask that $a$ and $b$ are not comparable in $T$ and that $d$ and $e$ are not comparable in $T$. Otherwise, a tree that is an interval can be starred according to this definition.}\Vincent{You are totally right!}
...
The reorientation~$R$ of~$\tc(T)$ defined by~$\rev(R) = \set{(w,d)}{a \le_T w <_T d} \cup \set{(w,e)}{b \le_T w < e}$ is transitively closed because~$d \not<_T e$ and~$e \not<_T d$, transitively coclosed because~$(u,w) \in \rev(R)$ implies~$(v,w) \in \rev(R)$ for any~$u <_T v <_T w$, and cyclic because~$a, d, b, e$ forms a directed cycle.
\Vincent{I use $a,d,b,e$}

The ornamentation~$O = O_R$ of~$T$ with~$O(d) = \set{w}{a \le_T w <_T d}$, $O(e) = \set{w}{b \le_T w <_T e}$, and~$O(w) = \{w\}$ for all~$w \in V \ssm \{d,e\}$ \Felix{$w\in V \ssm \set{c}{a \le_T c <_T d \textit{ or } b \le_T c <_T e}$}\Vincent{I need to define $O(w)$ for all~$w$...}  is cyclic.
...
We conclude that~$a, d, b, e$ forms a directed cycle in~$R'$ as well.
\Vincent{I use $a,d,b,e$}
Hence, $O$ is cyclic.
...
\Andrew{Do we need to assume here that $\I$ contains all singletons? Otherwise is it possible for $b = c$? Or does path intersection closed handle this somehow?}
\Vincent{Not sure to understand the issue. To avoid assuming that $\II$ contains singletons, I added~$|I \cap J| > 1$. I have added a sentence stating that $a,b,c,d$ are all distincts.}
\[
\pi_A \eqdef XbacdY
\quad
\pi_B \eqdef XacdbY
...
\Andrew{Why can't $g = b$ or $g = c$? Also were these intended to be on new lines or in an itemize/enumerate environment? It would make the cases easier to follow I think.}
\Vincent{$g = b$ is covered by the second case. $g = c$ is covered by the fourth case. I added an itemize, it is indeed much easier to read.}
\end{proof}
...
As~$S(I) \in J$ and~$S(I) < S(J)$, there exists~$p \in [q]$ and~$K \in \II$
\Felix{Should we briefly say that this $K$ is coming from the path intersection closedness?}
\Vincent{$K$ is coming from the formula, not from intersection closedness... I have added a sentence at the beginning to make clear where we actually use the assumptions...}
such that~$S_p(K) \in J$ and~$S(I) \in K$ and~$S(I) < S_p(K) \le S(J)$.
