...
For instance,
\begin{itemize}
\item for the directed graph~$\Dgraph$ of  \cref{fig:ornamentationsD}, the transitively biclosed reorientation poset is not even a lattice as observed in \cref{rem:biclosedNotLattice}, \vincent{What is the MacNeille completion of~$\AReori(\tc(\Dgraph))$ then?? This should be fairly easy to compute...}
...
\begin{proof}
Consider~$u,v$ as in \cref{lem:starredCharacterization}, and let~$a,b$ be two incoming neighbors of~$u$ and~$c,d$ be two outgoing neighbors of~$v$.
Let~$R$ be the reorientation of~$\tc(T)$ defined by~$\rev(R) = \{(a,d), (b,c), (v,c), (c,d)\}$, and let~$O = O_R$.
\vincent{todo}
