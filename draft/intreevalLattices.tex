...
\Jose[inline]{I don't see why replacing $u_k$ by $w$ still gives an induced cycle. What if the path between $d_k$ and $u_k$ does not pass through $w$ (and so $(d_k,w) \notin \tc(T)$)}
...
Note that
\begin{itemize}
\item if~$I , J \in \II$ and~$|I \cap J| > 1$ implies~$I \cap J \in \II$, then~$\II$ is path intersection closed,
\item if~$\II$ is path intersection closed, then ~$I,J \in \II$ and~$|I \cap J| > 1$ implies that there is~$K \in \II$ with~$I \cap J \subseteq K$.\Jose{This is always true taking $K = I$ or $K = J$ (independent of being path intersection closed).}
\end{itemize}
...
\begin{remark}
When~$T$ is a\jose{n oriented path\sout{line}}, 
\begin{itemize}
...
\item the star sparse condition \jose{\sout{is empty}holds trivially},
...
\Jose[inline]{Would it be more clear if we label these permutations so that \[\pi_A < \pi_B > \pi_C < \pi_D > \pi_E < \pi_F\]? That would make \cref{exm:pathIntersectionClosed} easier to verify for the reader, too.}
\[
\pi_A \eqdef XbacdY
\quad
\pi_B \eqdef XacdbY
...
\pi_F \eqdef XadbcY
\]
and let~$S_A \eqdef \asour{\pi_A}, \dots, S_F \eqdef \asour{\pi_F}$ denote the corresponding acyclic sourcings of~\jose{\sout{$T$}$\II$ or $\PP(T)$?} (where~${\pi \mapsto \jose{\sout{\aorn{\pi}}\asour{\pi}}}$ is the composition of $\pi \mapsto \areori{\pi}$ and $R \mapsto \asour{\pi}$).
...
If~$\ASour(\II)$ were a lattice, we \jose{\sout{therefore must}would}\Jose{Maybe its more clear to start the proof with $\ASour(\II)$ being a lattice and writing here "Since $\ASour(\II)$ is a lattice, we have..."} have an acyclic sourcing~$S_G$ of~$\II$ such that
\[
S_A \le S_G
\qquad
S_B \le S_G
...
If~$\ASour(\II)$ were a lattice, we therefore must have\Jose{same grammar question as in previous proof} an acyclic sourcing~$T$ of~$\II$ such that~$S^i \le T \le T^i$ for all~$i \in [p]$.
...
By symmetry, assume that~$w_i$ has two incoming (resp.~outgoing) arrows in this cycle if~$i$ is even (resp.~odd); see the figure below.
\begin{center}
\begin{tikzpicture}
\node[fill,circle,inner sep=1pt] (Z) at (-1,1.5) {};
\node[fill,circle,inner sep=1pt] (Y) at (-1,1.2) {};
\node[fill,circle,inner sep=1pt] (A) at (0,0.0) {};
\node[fill,circle,inner sep=1pt] (B) at (0,0.3) {};
\node[fill,circle,inner sep=1pt] (D) at (1,1.5) {};
\node[fill,circle,inner sep=1pt] (C) at (1,1.2) {};
\node[fill,circle,inner sep=1pt] (E) at (2,0.0) {};
\node[fill,circle,inner sep=1pt] (F) at (2,0.3) {};
\node[fill,circle,inner sep=1pt] (H) at (3,1.5) {};
\node[fill,circle,inner sep=1pt] (G) at (3,1.2) {};

\node at (-.55,1.5) {\scriptsize $w_{2j-2}$};
\node at (-.55,1.2) {\scriptsize $w'_{2j-2}$};
\node at (0.45,0.0) {\scriptsize $w_{2j-1}$};
\node at (0.45,0.3) {\scriptsize $w'_{2j-1}$};
\node at (1.3,1.5) {\scriptsize $w_{2j}$};
\node at (1.3,1.2) {\scriptsize $w'_{2j}$};
\node at (2.45,0.0) {\scriptsize $w_{2j+1}$};
\node at (2.45,0.3) {\scriptsize $w'_{2j+1}$};
\node at (3.45,1.5) {\scriptsize $w_{2j+2}$};
\node at (3.45,1.2) {\scriptsize $w'_{2j+2}$};

\node at (-2,.75) {$\cdots$};
\draw (-1.5,.75) to [out=10,in=270] (Y);
\draw (B) to [out=90,in=270] (Y) -- (Z);
\draw (A) -- (B) to [out=90,in=270] (C) -- (D);
\draw (E) -- (F) to [out=90,in=270] (C);
\draw (F) to [out=90,in=270] (G) -- (H);
\draw (3.5,.75) to [out=170,in=270] (G);
\node at (4,.75) {$\cdots$};
\end{tikzpicture}
\end{center}
...
\end{itemize}
Clearly, a lattice map is a quasi-lattice map, but the reverse is \jose{\sout{wrong}not true}.
...
Note that $\sour{O}^\II$ is just the restriction to~$\II$ of the sourcing~$\sour{O}$ of~$\PP(T)$ defined in \cref{def:Orn2Sour}.\Jose{Should we use a similar notation for the acyclic sourcings of $\II$ in the proof of \cref{lem:pathIntersectionClosed}?}
...
For any acyclic sourcing~$S$ of~$\II$, the ornamentations~$O$ of~$T$ such that~$\sour{O}^\II = S$ form an interval of the \jose{\sout{acyclic sourcing poset~$\ASour(\II)$}ornamentation lattice $\Orn(T)$} with minimal element~$\minorn{S}{\II}$ and maximal element~$\maxorn{S}{\II}$.
\end{lemma}
