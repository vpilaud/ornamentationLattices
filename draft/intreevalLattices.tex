...
\end{itemize}
Note that for the maximum, we had to consider a directed graph containing a cycle, due to the following statement.
\end{remark}
...
For any ornamentation~$O$ of a directed tree~$T$, the reorientation~$\maxreori{O}$ is the maximal reorientation~$R$ of~$\tc(T)$ with~$\orn{R} = O$.
\end{proposition}
...
This surjection behaves much nicer with respect to \cref{rem:Reori2OrnIntervals,rem:Reori2OrnSemilatticeMap}.

\begin{proposition}
\label{prop:Reori2OrnMinTC}
For any ornamentation~$O$ of~$D$, the reorientation~$\reori{O}$ is the minimal transitively closed reorientation~$R$ of~$\tc(T)$ with~$\orn{R} = O$.
\end{proposition}
...
Let $R_1$ and $R_2$ be two transitively closed reorientations of $\tc(D)$, and $O_\meet = \orn{R_1} \meet \orn{R_2}$.
We want to show that $O_\meet = \orn{R_1 \meet R_2}$.

Let $u \in O_\meet(v)$.
Then, there exists a path from $u$ to $v$ in $O_\meet(v) \subseteq \orn{R_1}(v) \cap \orn{R_2}(v)$.
Let~$u = w_0, w_1, \dots, w_p = v$ denote the vertices along this path.
By definition of $\orn{R_1}$ and $\orn{R_2}$, there is a path from each $w_i$ to $v$ in both $\rev(R_1)$ and $\rev(R_2)$.
Since $R_1$ and $R_2$ are transitively closed, each arrow $(w_i,v)$ of $\tc(D)$ is in both $\rev(R_1)$ and $\rev(R_2)$;
and therefore in $\rev(R_1 \meet R_2) = \rev(R_1) \cap \rev(R_2)$ (\cref{lem:rcl-subsemilattice}).
...
Now let $u \in \orn{R_1 \meet R_2}(v)$ and
let $u = w_0, w_1, \dots, w_p = v$ denote the vertices along a path from $u$ to $v$ in $\orn{R_1 \meet R_2}(v)$.
...
We have shown that $O_\meet \geq \orn{R_1 \meet R_2}$, and therefore they are equal.
\end{proof}
