...
\newcommand{\Jose}[1]{\todo[size=\tiny,color=blue!30]{ #1 \\ \hfill --- J.}\,}
...
In particular, \jose{whenever the edges of $E$ are increasing,} it turns out that the acyclic reorientation poset~$\AReori(E)$ is isomorphic to the transitive closure of the graph of~$\simplex_E$, oriented in the direction~$\b{\omega}$.\Jose{So that inclusion of reversed arrows agrees with the direction $\b{\omega}$}
...
An \defn{ornamentation} of~$D$ is an assignment~$O$ of \jose{one} ornament\jose{\sout{s}} at each vertex of~$V$ with the nesting condition that $u \in O(v)$ implies~$O(u) \subseteq O(v)$.
...
Unfortunately, \jose{even when $\ASour(\II)$ is a lattice,} the map~$\pi \mapsto \asour{\pi}$\Jose{In the previous paragraph we used $\pi \mapsto \sour{\pi}$ instead of $\asour{\pi}$, which one should we keep?} from permutations of~$[n]$ to acyclic sourcings of~$\II$ is not \jose{necessarily} a quasi-lattice map anymore (its fibers are not intervals in general).
...
We use instead a natural quasi-lattice map from the ornamentation lattice~$\Orn(T)$ to the acyclic sourcing poset~$\ASour(\II)$ to obtain the following \jose{complete} characterization.
...
The path intersection closed and path star sparse conditions are slightly technical\jose{, we defer the details  \sout{conditions whose details are deferred}} to \cref{sec:intreevalHypergraphicPosets}.
Let us just mention that they clearly specialize to that of~\cite{BergeronPilaud} when~$T$ is a path, as\jose{, in this case, \sout{the}} path intersection closedness boils down to intersection closedness \jose{\sout{when~$T$ is a path}}, and \jose{\sout{the}} path star sparseness always holds \jose{\sout{when~$T$ is a path}}.
