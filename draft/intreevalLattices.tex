...
\pagebreak
For instance, when~$E = K_n$ is the complete graph, \Jose{We used $\binom{[n]}{2}$ for the complete graph before (though it was in the context of hypergraphs), should we use the same notation in both places?}
\begin{itemize}
...
Alternatively, each bubble is an ornament at the largest vertex it contains.\Jose{I added this sentence to have a color-independent explanation of the pictures}
...
In a directed tree, a nonempty subset $U \subseteq \lessin{T}{v}$ is an ornament if and only if for all $u \in U$, the (vertex set of the) unique path from $u$ to $v$ in $T$ is contained in $U$. This property is closed under intersection.
\end{proof}
...
Consider now~$u', v' \in V$ such that~$u' \in M_P(v')$.
If~$u <_T v' \le_T v$, then~$M_P(u') = \lessin{T}{u'} \ssm \lessin{T}{u} \subseteq \lessin{T}{v'} \ssm \lessin{T}{u} = M_P(v')$; observe that the first equality holds even if $u \not\le_T u'$, since in this case $\lessin{T}{u'} \ssm \lessin{T}{u} = \lessin{T}{u'}$.\Jose{The first time I read this sentence it seemed to me that we were assuming that $u'$ is also in the path, so I added the extra clarification.}
Otherwise, $M_P(u') \subseteq \lessin{T}{u'} \subseteq \lessin{T}{v'} = M_P(v')$.
\end{proof}
...
Write~$C = (d_1, u_1, \dots, d_k, u_k)$ where~$(d_i, u_i)$ and~\jose{\sout{$(u_i, d_{i+1})$} $(d_{i+1},u_i)$} are in~$\tc(T)$\Jose{or (if we don't make the previous change), are in $R$} for all~$i \in [k]$.
...
We denote by~$X(T,u,v,\II)$ the \andrew{(undirected)} graph on~$u_1, \dots, u_k,\; v_1, \dots, v_\ell$ with an edge joining~$u_i$ and~$v_j$ if and only if there is~$I \in \II$ containing both~$u_i$ and~$v_j$.
...
Assume by means of contradiction that there are~$I,J\in \II$ with~$|I\cap J|> 1$ such that $\min(I) <_T \min(I\cap J)$ and  $\max J <_T \andrew{\text{ should be } >_T} \max (I\cap J)$, and there is no~$K \in \II$ such that $\min(J) \le_T \min(K) \le_T \min(I\cap J)$ and ${\max(I\cap J) \le_T \max(K) \le_T \max(I)}$.
...
Consider the permutations \Andrew{Do we need to assume here that $\I$ contains all singletons? Otherwise is it possible for $b = c$? Or does path intersection closed handle this somehow?}
\[
\pi_A \eqdef XbacdY
\quad
\pi_B \eqdef XacdbY
...
As these four cases are all excluded, we reached a contradiction. \Andrew{Why can't $g = b$ or $g = c$? Also were these intended to be on new lines or in an itemize/enumerate environment? It would make the cases easier to follow I think.}
\end{proof}
...
As~$S(I) \in J$ and~$S(I) < S(J)$, there exists~$p \in [q]$ and~$K \in \II$\Felix{Should we briefly say that this $K$ is coming from the path intersection closedness?} such that~$S_p(K) \in J$ and~$S(I) \in K$ and~$S(I) < S_p(K) \le S(J)$.
