...
\Vincent{I changed again as I feel my path is needed...}
If~$O_1 \ne O$, then there is~$x \in O(v) \ssm O_1(v)$ and let~$y$ be the last vertex not in~$O_1(v)$ along the path from~$x$ to~$v$ in~$O(v)$.
As~$y \in O(v) \ssm O_1(v) \subseteq O_2(v) \ssm  O_1(v) = O_1(u)$.
Hence, the directed path from~$x$ to~$v$ passes through~$u$, and we have~$u = y \in O(v)$.
...
\Antoine{No need the graph to be a tree. Should we put the result earlier in the article?}
