...
When~$D$ is a rooted increasing tree (or even any unstarred tree), we show that the acyclic sourcing poset is isomorphic to the ornamentation lattice, and they form a lattice quotient of the acyclic reorientation poset.
As a consequence, we obtain a polytopal realization of the ornamentation lattices of rooted (or even unstarred) increasing trees, answering an open question of C.~Defant and A.~Sack.
When~$D$ is an increasing tree, we show that the ornamentation lattice is the MacNeille completion of the acyclic sourcing poset.
Finally, still when~$D$ is an increasing tree, we use the ornamentation lattice to characterize the subhypergraphs of the path hypergraph~$\PP(D)$ whose acyclic sourcing poset is a lattice.
\end{abstract}
...
A sourcing~$S$ of~$\HH$ is \defn{acyclic} if there is no directed cycle in the directed graph with edges~$\set{(h,S(H))}{h \in H \in \HH}$.
The \defn{(acyclic) sourcing poset} of~$\HH$ is the poset of (acyclic) sourcings of~$\HH$ ordered by componentwise comparison, meaning~$S(H) \le S'(H)$ for all~$H \in \HH$.
While the sourcing poset~$\Sour(\HH)$ is clearly a lattice (as a product intervals), the acyclic sourcing poset~$\ASour(\HH)$ is not always a lattice.
%When~$\HH$ is the hypergraph of all intervals of~$[n]$, the acyclic sourcing poset~$\Sour(\HH)$ is the Tamari lattice.
The question to characterize the hypergraphs~$\HH$ whose acyclic sourcing posets~$\ASour(\HH)$ are lattices has been studied for specific families of hypergraphs, notably for graphs~\cite{Pilaud-acyclicReorientationLattices}, interval hypergraphs~\cite{BergeronPilaud}, and (partially for) graph associahedra~\cite{BarnardMcConville}.
...
In particular, it turns out that the acyclic sourcing poset~$\ASour(\HH)$ is the transitive closure of the graph of the hypergraphic polytope~$\simplex_\HH$, oriented in the standard direction~$\b{\omega} \eqdef (n-1, n-3, \dots, 3-n, 1-n)$.

For instance, 
\begin{itemize}
\item if~$\HH = \binom{[n]}{2}$ is the complete graph (or any hypergraph containing it), then~$\simplex_\HH$ is the \defn{permutahedron} and $\ASour(\HH)$ is the \defn{weak order on permutations},
\item if~$\HH = \set{[i,j]}{1 \le i \le j \le n}$ is the complete interval hypergraph, then~$\simplex_\HH$ is \mbox{J.-L.~Loday's} \defn{associahedron}~\cite{ShniderSternberg,Loday} and~$\ASour(\HH)$ is the \defn{Tamari lattice on binary trees}~\cite{Tamari}.
\end{itemize}
...
The \defn{(acyclic) reorientation poset} of~$E$ is the set of (acyclic) reorientations of~$E$ ordered by inclusion of reversion sets, meaning~$\rev(R) \subseteq \rev(R')$.
...
In particular, it turns out that the acyclic reorientation poset~$\AReori(E)$ is isomorphic to the transitive closure of the graph of~$\simplex_E$, oriented in the direction~$\b{\omega}$.
...
For instance, when~$E$ is the complete graph,
\begin{itemize}
\item $\simplex_E$ is the permutahedron and~$\AReori(E)$ is the weak order on permutations,
\item the associahedron of~$E$ is Loday's associahedron and the Tamari lattice of~$E$ is the Tamari lattice.
\end{itemize}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\para{Ornamentation lattices.}
Consider a directed graph~$D$ on~$V$ and a vertex~$v$ of~$D$.
...
The \defn{(acyclic) ornamentation poset} of~$D$ is the poset of (acyclic) ornamentations of~$D$ ordered by componentwise inclusion, meaning~$O(v) \subseteq O'(v)$ for all~$v \in V$.
The ornamentation poset~$\Orn(D)$ is always a lattice, but that the acyclic ornamentation poset~$\Orn(D)$ is not always a lattice.
...
We will answer this question for rooted trees and discuss it for arbitrary directed graphs.
...
The general purpose of the present paper is to connect these three extensions, in particular for directed trees.

In \cref{sec:general}, we first present in more details the reorientation lattice~$\Reori(E)$, the sourcing lattice~$\Sour(\HH)$, and the ornamentation lattice~$\Orn(D)$ mentioned above, and we connect them as follows.

\begin{proposition}
Given a directed graph~$D$, denote by~$\tc(D)$ its transitive closure and by~$\PP(D)$ its path hypergraph, whose hyperedges are the (vertex sets of) directed paths in~$D$.
...
.
\]
Moreover, $R \mapsto \orn{R}$ (resp.~$S \to \orn{S}$) restricts to a meet semilattice morphism from the transitively closed reorientation lattice of~$\tc(D)$ (resp.~sourcing lattice of~$\PP(D)$) to the ornamentation lattice~of~$D$.
\end{proposition}

In \cref{sec:acyclic}, we focus on acyclicity.
We consider the acyclic reorientation poset~$\AReori(E)$, the acyclic sourcing poset~$\ASour(\HH)$, and define the acyclic ornamentation poset~$\AOrn(D)$, and we connect them as follows.
...
.
\]
In particular, the acyclic sourcing poset~$\ASour(\PP(D))$ can be seen as a subposet of the ornamentation poset~$\Orn(D)$.
\end{proposition}

These connections become particularly interesting when the ground directed graph~$D$ is an increasing tree~$T$, and even more if it is rooted (or unstarred, see below).
...
Observing that all join or meet irreducible ornamentations of~$T$ are acyclic, we obtain the following strong connection between the acyclic sourcing poset~$\ASour(\PP(T))$ and the ornamentation lattice~$\Orn(T)$.

\begin{theorem}
\label{thm:main1}
For any increasing tree~$T$ on~$[n]$, the ornamentation lattice~$\Orn(T)$ is the MacNeille completion of the acyclic sourcing poset~$\ASour(\PP(T))$.
\end{theorem}

We then consider a special class of increasing trees, containing in particular all rooted trees.
...
\item all ornamentations of~$T$ are in fact acyclic, so that~$\ASour(\PP(T)) \sim \AOrn(T) = \Orn(T)$,
...
For an interval hypergraph~$\II$ containing all singletons, it was proved in~\cite[Thm~A]{BergeronPilaud} that the acyclic sourcing poset~$\ASour(I)$ is a lattice if and only if~$\II$ is closed under intersection, meaning that~$I,J \in \II$ implies~$I \cap J \in \II$.
...
The approach of~\cite{BergeronPilaud} is to show that the map~$\pi \mapsto \sour{\pi}$ sending a permutation of~$[n]$ to an acyclic sourcing of~$\II$ is a \defn{quasi-lattice map}, meaning that the fiber of each acyclic sourcing~$S$ of~$\II$ forms an interval~$[\projDown{S}, \projUp{S}]$ of the weak order and that~$S \le S'$ in~$\ASour(I)$ implies that~$\projDown{S} \le \projUp{S'}$.
...
For an increasing tree~$T$ on~$[n]$, we call \defn{intreeval hypergraph} of~$T$ any subhypergraph~$\II$ of the path hypergraph~$\PP(T)$, meaning any collection of (vertex sets of) directed paths in~$T$.
Unfortunately, the map~$\pi \mapsto \asour{\pi}$ from permutations of~$[n]$ to acyclic sourcings of~$\II$ is not a quasi-lattice map anymore (its fibers are not intervals in general).
Moreover, we can neither use the map~$R \mapsto \asour{R}$ from acyclic reorientations of~$\tc(T)$ to acyclic sourcings of~$\II$ as the acyclic reorientation poset~$\AReori(\tc(T))$ is not a lattice in general, in contrast to~\cref{thm:main2}.
We use instead a natural quasi-lattice map from the ornamentation lattice~$\Orn(T)$ to the acyclic sourcing poset~$\ASour(\II)$ to obtain a characterization.
...
The path intersection closed and path star sparse conditions are slightly technical conditions whose details are deferred to \cref{sec:intreevalHypergraphicPosets}.
Let us just mention that they clearly specialize to that of~\cite{BergeronPilaud} when~$T$ is a path, as the path intersection closedness boils down to intersection closedness when~$T$ is a path, and the path star sparseness always holds when~$T$ is a path.
...
We clearly define ornamentations by
\begin{enumerate}[(i)]
\item sending each vertex~$v \in V$ to the singleton~$\{v\}$, 
\item sending each vertex~$v \in V$ to the set~$\triangle_D(v)$ of vertices with a path to~$v$ in~$D$,
...
\begin{example}
The ornamentation lattice of an $n$-element path is isomorphic to the Tamari lattice on binary trees with $n$ nodes (see \eg \cref{fig:ornamentationsNI}\,(right)).
...
%\vincent{Maybe we should extend the previous two remarks to proper statements with proofs...}
%
%\begin{remark}
...
%
%\begin{proposition}
%\label{prop:distributiveOrnamentationLattices}
%The ornamentation lattce~$\Orn(D)$ is distributive if and only if~$D$ contains no path of length~$2$ (in which case, $\Orn(D)$ is actually a boolean lattice).\end{proposition}
%
%\begin{proof}
%\vincent{todo}
%\end{proof}
%
%\begin{proposition}
%\label{prop:semidistributiveOrnamentationLattices}
%The ornamentation lattce~$\Orn(D)$ is semidistributive if and only if~$D$ is a directed tree.
%\end{proposition}
%
%\begin{proof}
%\vincent{todo}
...
\begin{remark}
Observe however that~$\Rcl(\tc(D)) \cap \Rco(\tc(D))$ may fail to be a lattice. \vincent{even when~$D$ is a directed path?}
For instance, consider the following four transitively closed and coclosed reorientations:
%\[
%\begin{array}{cccc}
...
\]
Then~$R_1, R_2 < R_3, R_4$ and there is no transitively closed and coclosed reorientation~$R$ such that~$R_1, R_2 < R < R_3, R_4$.
\end{remark}
...
\begin{remark}
Note that the image of the transitively coclosed reorientations by the map~$R \mapsto \orn{R}$ does not cover all ornamentations of~$D$.
...
\vincent{I decided quite late to introduce these notations. There are certainly some places where they could be used to simplify the phrasing. I will check that during proofreading.}
...
%\vincent{I am still not sure that we really want this statement.}
%\end{lemma}
%
%\begin{proof}
%This follows from \cref{lem:coverRelationsOrn}.
%\vincent{Maybe we just want the proof here.}
%\end{proof}
...
%\]
Observe first that~$J_P$ (resp.~$M_P$) is indeed join (resp.~meet) irreducible as it covers (resp. is covered by) a unique ornamentation given by
%removing (resp.~adding)~$u$ from~$J_P(v)$ (resp.~to~$M_P(w)$ for all~$w \in P \ssm \{u\}$).
removing~$u$ from~$J_P(v)$ (resp.adding!$\Delta_T(u)$ to $M_P(v)$).
\jose{I made a change (Previous version commented). I think otherwise we'd break the ornamentation property. I also seems consistent to the second paragraph below (construction of $M_u^v$)}
...
\vincent{This section must be cleaned...}
...
We say that an increasing tree~$T$ on~$[n]$ is \defn{starred} if it satisfies the equivalent conditions of \cref{lem:starredFreeTrees} and \defn{unstarred} otherwise.
...
Given any subset~$U \subset [n]$, the subgraph of~$\tc(T)$ induced by~$U$ contains no induced alternating cycle, hence no induced cycles of length at least~$4$.
...
\begin{corollary}
If~$T$ is an unstarred tree, then the Hasse diagram of the ornamentation lattice~$\Orn(T)$ is isomorphic to the graph of the path hypergraphic polytope~$\simplex_{\PP(D)} \eqdef \sum_{P \in \PP(T)} \simplex_P$ oriented in the direction~$\omega$.
\end{corollary}
...
\vincent{This section needs illustrations... I will do that later.}
...
We say that an intreeval hypergraph~$\II$ of an increasing tree~$T$ is \defn{path intersection closed} if for all~$I,J\in \II$ with~$|I\cap J|> 1$ such that $\min(I) <_T \min(I\cap J)$ and  $\max J >_T \max (I\cap J)$, there exists an~$K \in \II$ such that $\min(J) \le_T \min(K) \le_T \min(I\cap J)$ and ${\max(I\cap J) \le_T \max(K) \le_T \max(I)}$.
\end{definition}
...
\begin{remark}
There is an explicit formula for the meet and join.
\vincent{todo}
\end{remark}
...
Let~$\II$ be an intreeval hypergraph of an increasing tree~$T$.
If~$\ASour(\II)$ is a lattice, then $\II$ is path star sparse (\cref{def:pathStarSparse}).
\end{lemma}
...
Let~$\II$ be a path star sparse (\cref{def:pathStarSparse}) intreeval hypergraph of an increasing tree~$T$.
...
To prove the reverse implication of \cref{thm:intreevalLattices}, we will use a quasi-lattice quotient as defined in~\cite[Rem.~4.14]{BergeronPilaud}.
...
Let~$T$ be an increasing tree, $\II$ be a path star sparse intreeval hypergraph of~$T$, and~$O$ be an ornamentation of~$T$.
...
Let~$\II$ be a path intersection closed (\cref{def:pathIntersectionClose}) and path star sparse (\cref{def:pathStarSparse}) intreeval hypergraph of an increasing tree~$T$.
