...
\item sending each vertex~$v \in V$ to the set~$\triangle_T(v)$ of vertices with a path to~$v$ in~$D$,
...
We say that an intreeval hypergraph~$\II$ of an increasing tree~$T$ is \defn{path intersection closed} if for all~$I,J\in \II$ with~$|I\cap J|> 1$ such that $\min(I) < \min(I\cap J)$ and  $\max J > \max (I\cap J)$, there exists an~$K \in \II$ such that $\min(J) \le \min(K) \le \min(I\cap J)$ and $\max(I\cap J) \le \max K \le \max(I)$.
\end{definition}
...
Assume by means of contradiction that there are~$I,J\in \II$ with~$|I\cap J|> 1$ such that $\min(I) < \min(I\cap J)$ and  $\max J < \max (I\cap J)$, and there is there no~$K \in \II$ such that $\min(J) \le \min(K) \le \min(I\cap J)$ and $\max(I\cap J) \le \max K \le \max(I)$.
...
This would imply that~$\min(J) \le \min(K) \le b$ and $c \le \max K \le \max(I)$, contradicting our assumption on~$I$ and~$J$.
...
Let~$T$ be a directed tree, $\II$ be a path star sparse intreeval hypergraph of~$T$, and~$O$ be an ornamentation of~$T$.
...
%Here, it is convenient to assume that the intreeval hypergraph~$\II$ contains all singletons~$\{v\}$ for~$v \in V$.
...
We conclude that~$\minorn{S}{I}(v) \subseteq \maxorn{S}{J}(v)$ for any~$v \in V$, hence that~$\minorn{S}{I} \le \maxorn{S}{J}$.
\end{proof}
...
\begin{proof}
Consider first an ornamentation~$O$ of~$T$ such that~$\sour{O}^\II = S$ and let~$v \in V$ and~$I \in \II$. Then
\begin{itemize}
...
\end{itemize}
In all cases, $\minorn{S}{I}(v) \subseteq O(v) \le \maxorn{S}{I}(v)$ for any~$v \in V$ and~$I \in \II$.
...
For any acyclic sourcings~$S_1$ and~$S_2$ of~$\II$, if~$S_1 \le S_2$ then~$\minorn{S_1}{\II} \le \minorn{S_2}{\II}$.
\end{lemma}

\begin{proof}
Assume by means of contradiction that~$S_1 \le S_2$ and~$\minorn{S_1}{\II} \not\le \minorn{S_2}{\II}$.
Since~$\minorn{S_1}{\II} \le \minorn{S_2}{\II}$, there exist~$I, J \in \II$ such that~$\minorn{S_1}{I} \not\le \minorn{S_2}{J}$.
...
As~$\II$ is path intersection closed, there exists~$K \in \II$ such that~$\min(I) \le \min(K) \le \min(I\cap J)$ and $\max(I\cap J) \le \max K \le \max(J)$.
\vincent{todo: almost finished}
\vincent{todo: change the role of $I$ and~$J$ in the definition of path intersection closed?}
\end{proof}
...
%\begin{definition}
%Let~$\II$ be an intreeval hypergraph of an increasing tree~$T$, containing all singletons~$\{v\}$ for~$v \in V$.
%For an acyclic sourcing~$S$ of~$\II$, we denote by~$\minorn{S}$ and~$\maxorn{S}$ the two maps on~$V$ defined by
%\[
%\minorn{S}(v) \eqdef \bigcup_{\substack{v \in I \in \II \\ S(I) = v}} I \cap [v]
%\qquad\text{and}\qquad
%\maxorn{S}(v) \eqdef \triangle_T(v) \ssm \bigcup_{\substack{v \in I \in \II \\ S(I) < v}} I \cap [S(I)].
%\]
%\end{definition}
%
%\begin{lemma}
%For any acyclic sourcing~$S$ of~$\II$, the maps~$\minorn{S}$ and~$\maxorn{S}$ are both ornamentations~of~$T$.
%\end{lemma}
%
%\begin{proof}
%We have~$v \in \minorn{S}(v)$ (since~$v \in \{v\} \in \II$ and~$S(\{v\}) = v$).
%Let~$u \in \minorn{S}(v)$.
%Consider~$I \in \II$ such that~$\{u,v\} \subseteq I$ and~$S(I) = v$.
%For any~$u \in J \in \II$ with~$S(J) = u$, consider the directed path~$K$ from~$\min(J)$ to~$v$.
%If~$S(K) < u$, then~$S(K) \in J \ssm \{S(J)\}$ and~$S(J) \in K \ssm S(K)$.
%If~$u \le S(K) < v$, then~$S(K) \in I \ssm \{S(I)\}$ and~$S(I) \in K \ssm S(K)$.
%Since~$S$ is acyclic, we conclude that~$S(K) = v$, and thus that~$J \cap [u] \subseteq K \subseteq \minorn{S}(v)$.
%We conclude that~$\minorn{S}(u) \subseteq \minorn{S}(v)$.
%
%We have~$v \in \maxorn{S}(v)$ (since~$v \in \triangle_T(v)$ and~$v \notin \triangle_T(u)$ for~$u < v$).
%Let~$u \in \maxorn{S}(v)$
%
%\vincent{todo}
%\end{proof}
%
%\begin{lemma}
%For any acyclic sourcing~$S$ of~$\II$, we have~$\minorn{S} \le \maxorn{S}$.
%\end{lemma}
%
%\begin{proof}
%Let~$v \in I \in \II$ such that~$S(I) = v$.
%Note that~$I \cap [v] \subseteq \triangle_T(v)$.
%Moreover, for any~$v \in J \in \II$ such that~$S(J) < v$, we have~$S(J) \notin I$ by acyclicity of~$S$, thus~$I \cap (J \cap [S(J)]) = \varnothing$.
%We conclude that~$I \subseteq \maxorn{S}(v)$.
%Therefore~$\minorn{S}(v) \le \maxorn{S}(v)$ for any~$v \in V$, hence~$\minorn{S} \le \maxorn{S}$.
%\end{proof}
%
%\begin{lemma}
%For any acyclic sourcing~$S$ of~$\II$, the ornamentations~$O$ of~$T$ such that~$\sour{O}^\II = S$ form an interval of the acyclic sourcing poset~$\ASour(\II)$ with minimal element~$\minorn{S}$ and maximal element~$\maxorn{S}$.
%\end{lemma}
%
%\begin{proof}
%Consider first an ornamentation~$O$ of~$T$ such that~$\sour{O}^\II = S$ and~$v \in V$. Then
%\begin{itemize}
%\item For any~$v \in I \in \II$ such that~$S(I) = v$, we have~$\min(I) \in O(v)$ by definition of~$\sour{O}^\II = S$, and thus~$I \cap [v] \subseteq O(v)$ since~$T$ is a tree.
%We thus obtain that~$\minorn{S}(v) \subseteq O(v)$.
%\item For any~$v \in I \in \II$ such that~$S(I) < v$, we have~$\min(I) \in O(S(I)) \ssm O(v)$ by definition of~$\sour{O}^\II = S$, hence~$(I \cap [S(I)]) \cap O(v) = \varnothing$ since~$O$ is an ornamentation.
%We thus obtain that~$O(v) \subseteq \maxorn{S}(v)$.
%\end{itemize}
%As~$\minorn{S}(v) \subseteq O(v) \le \maxorn{S}(v)$ for any~$v \in V$, we obtain that~$\minorn{S} \subseteq O \le \maxorn{S}$.
%
%Conversely, consider an ornamentation~$O$ of~$T$ such that~$\minorn{S} \subseteq O \le \maxorn{S}$ and let~$I \in \II$ and~$v \eqdef S(I)$. Then
%\begin{itemize}
%\item As~$v \in I \in \II$ and~$S(I) = v$, we have~$I \cap [v] \subseteq \minorn{S}(v) \subseteq O(v)$, hence~$v \le \sour{O}^\II(I)$ by definition~of~$\sour{O}^\II$.
%\item Conversely, for any~$v < w \in I$, we have~$w \in I \in \II$ with~$S(I) = v < w$ so that~$O(w) \subseteq \maxorn{S}(w) \subseteq \triangle_T(w) \ssm (I \cap [v])$. Hence, $\min(I) \notin O(w)$, so that~$\sour{O}^\II(I) \ne w$. We thus obtain that~$\sour{O}^\II(I) \le v$
%\end{itemize}
%We conclude that~$\sour{O}^\II(I) = S(I)$ for all~$I \in \II$, hence that~$\sour{O}^\II = S$.
%\end{proof}
