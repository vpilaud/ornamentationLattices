...
\item there are distinct $a,b,c,d,e \in V$ such that~$(a,c)$, $(b,c)$, $(c,d)$, and~$(c,e)$ are all in~$\tc(T)$, \Felix{We need to ask that $a$ and $b$ are not comparable in $T$ and that $d$ and $e$ are not comparable in $T$. Otherwise, a tree that is an interval can be starred accroding to this definition.}\Vincent{You are totally right!}
