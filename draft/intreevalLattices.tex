...
The path intersection closed and star sparse conditions are slightly technical conditions whose details are deferred to \cref{sec:intreevalHypergraphicPosets}.
Let us just mention that they clearly specialize to that of~\cite{BergeronPilaud} when~$T$ is a path, as the path intersection closedness boils down to intersection closedness when~$T$ is a path, and the star sparseness always holds when~$T$ is a path.
