...
We denote by~$\rev(S)$ the set of pairs~$(u,v) \in V^2$ such that there is a directed path~$P \in \PP(D)$ from~$u$ to~$v$ with~$S(P) = v$.
...
We start with a simple observation about \cref{def:Sour2Reori} .
...
Despite \cref{lem:revTCAcyclic}, the reorientation~$\reori{S}$ of~$\tc(D)$ defined in \cref{def:Sour2Reori} by~$\rev(\reori{S}) = \rev(S)$ is not always acyclic even when the sourcing~$S$ of~$\PP(D)$ is acyclic.
