...
In this section, we focus on the specific situation when the ground graph~$D$ is a directed tree~$T$ (but not necessarily a rooted tree).
...
\begin{proof}
Let~$u, v, w \in V$ such that~$(u,v)$ and~$(v,w)$ are in~$\tc(T)$.
...
Conversely, assume that~$(u,w) \in \rev(\reori{O})$. Then~$u \in O(w)$. As~$T$ is a tree, the complete path from~$u$ to~$w$ is in~$O(w)$, hence~$v \in O(w)$ and thus $(v,w) \in \rev(\reori{O})$.
...
\begin{lemma}
Consider an induced alternating cycle~$C = d_1 \rightarrow u_1 \leftarrow d_2 \rightarrow u_2 \dots d_k \rightarrow u_k \leftarrow d_1$ in~$\tc(T)$ for a directed tree~$T$.
Then~$k = 2$ and there exists~$v \in V$ such that~$(d_1, v)$, $(d_2,v)$, $(v,u_1)$, and~$(v,u_2)$ are in~$\tc(T)$.
\end{lemma}
...
In this section, we discuss the specific situation when the ground graph~$D$ is a rooted tree~$T$ (with increasing edges all oriented toward the root).

\begin{proposition}
\label{prop:rootedTrees1}
For any ornamentation~$O$ of a rooted tree~$T$, the reorientation~$\reori{O}$ of~$\tc(T)$ and the sourcing~$\sour{O}$ of~$\PP(T)$ are both acyclic.
...
For a rooted tree~$T$, the acyclic reorientation poset~$\AReori(\tc(T))$, the acyclic sourcing poset~$\ASour(\PP(T))$, and the acyclic ornamentation poset~$\AOrn(T)$ are all lattices.
\end{proposition}
...
For a rooted tree~$T$, the map $R \mapsto \aorn{R}$ is a surjective lattice map, hence the lattices~$\ASour(\PP(T)) \simeq \AOrn(T) = \Orn(T)$ are lattice quotients~of~$\AReori(\tc(T))$.
\end{proposition}
