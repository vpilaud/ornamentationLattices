...
In fact, these boolean lattices are the only distributive ornamentation lattices.
\end{example}
...
\begin{remark}
Observe however that~$\Rcl(\tc(D)) \cap \Rco(\tc(D))$ may fail to be a lattice when~$D$ is not a tree.
...
\vincent{$\Rcl(\tc(D)) \cap \Rco(\tc(D))$ is a lattice when~$D$ is a directed tree. Not sure where to place that.}
\end{remark}
...
\subsection{Semidistributivity and canonical join decompositions}
...
Let~$O_1, O_2$ be two ornamentations of a directed tree~$T$ which form a cover relation~$O_1 \lessdot O_2$, let~$u,v \in V$ such that~$u \notin O_1(v)$ and $O_2(v) = O_1(u) \cup O_1(v)$ while~$O_1(w) = O_2(w)$ for all~${w \in V \ssm \{v\}}$ (by \cref{lem:coverRelationsOrnT}), and let~$P$ denote the directed path from~$u$ to~$v$ in~$T$.
Then~$J_P$ (resp.~$M_P$) is the unique minimal element of~$\set{O \in \Orn(T)}{O \not\le O_1 \text{ but } O \le O_2}$ (resp.~of~$\set{O \in \Orn(T)}{O_1 \le O \text{ but } O_2 \not\le O}$).
\end{lemma}
...
\item the canonical join (resp.~meet) representation of an ornamentation~$O$ is~$O = \bigJoin J_P$ (resp.~$O = \bigMeet M_P$) where~$P$ ranges over ??? \vincent{todo} 
...
\begin{remark}
Observe that the ornamentation lattice~$\Orn(\Dgraph)$ of the diamond graph~$\Dgraph$ represented in \cref{fig:ornamentationsD} is not semidistributive.
\end{remark}
...
\begin{proof}
This follows from \cref{lem:irreduciblesAcyclicT,thm:OrnSemidistributiveT}.
\end{proof}
...
For a directed tree~$T$, the ornamentation lattice~$\Orn(T)$ is the MacNeille completion of the acyclic ornamentation poset~$\AOrn(T)$ (or equivalently of the acyclic sourcing poset~$\ASour(\PP(T))$).
\end{theorem}
...
We show \cref{thm:main2} in \cref{subsec:OrnUT}.
Finally, we discuss enumeration properties of ornamentations of brooms in \cref{subsec:brooms}.
