...
\begin{lemma}
\label{lem:starred}
For a directed tree~$T$, any induced alternating cycle in~$\tc(T)$ is of the form~$(a,c,b,d)$ where~$a,b,c,d$ are starred.
\end{lemma}

\begin{proof}
Let~$C$ be an alternating induced cycle in~$\tc(T)$.
...
\begin{enumerate}[(i)]
\item $\tc(T)$ contains an alternating induced cycle.
...
(ii) $\Rightarrow$ (i). Let~$a,b,c,d,e$ as in~(ii). Then~$(a,d,b,e)$ forms an alternating induced cycle in~$\tc(T)$.
...
An induced oriented cycle in~$\reori{O}$ is an induced cycle in~$\tc(T)$ by~\cref{lem:Orn2ReoriDT}.
This would imply that~$T$ is starred by \cref{lem:starred}.
...
Given any subset~$U \subset V$, the subgraph of~$\tc(T)$ induced by~$U$ contains no alternating induced cycle, hence no induced cycles of length at least~$4$.
...
We can reformulate \cref{prop:rootedTrees} using the language of~\cite{Pilaud-acyclicReorientationLattices}: if~$T$ is an unstarred tree, then
\begin{enumerate}
