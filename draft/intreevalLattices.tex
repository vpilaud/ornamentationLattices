...
Finally, still when~$D$ is an increasing tree, we use the ornamentation lattice of~$D$ to characterize the subhypergraphs of the path hypergraph~$\PP(D)$ whose acyclic sourcing poset is a lattice. \andrew{Do you think ``hypergraphic poset'' or ``acyclic sourcing poset'' is better for the abstract? I'm thinking in terms of what people are more likely to be familiar with. I also think we should probably mention the geometry somewhere in the abstract.}
\end{abstract}
...
The path intersection closed and star sparse conditions are slightly technical; we defer the details to \cref{sec:intreevalHypergraphicPosets}. \Andrew{I think we should mention somewhere that being intersection closed is a stronger condition than being path intersection closed. In particular, for a rooted tree, being intersection closed is sufficient to be a lattice.}
...
We define the reorientation lattice~$\Reori(\tc(D))$ (\cref{subsec:reorientations}), the sourcing lattice~$\Sour(\PP(D))$ (\cref{subsec:sourcings}), and the ornamentation lattice~$\Orn(D)$ (\cref{subsec:ornamentations}) \Felix{Would change the order we introduce the sections}, and define natural surjective poset morphisms:
\[
\begin{array}{rcl}
	\Reori(\tc(D)) & \surjection & \Orn(D) \\
	R & \longmapsto & \orn{R}
...
\item If~$U,U' \in \Orn(v \in D)$, then~$U \cup U' \in \Orn(v \in D)$, while $U \cap U'$ is not necessarily~in~${\Orn(v \in D)}$. \Andrew{Do we address this ``not necessarily'' condition anywhere?} 
...
The isomorphism is obtained through tubings (or equivalently bracket vectors). \Andrew{I think this is a little vague. Perhaps we should say something like, ``The isomorphism is identical to the construction of bracket vectors in~\cite{HuangTamari}. I'm not sure the reader will know what we mean by tubings.}
\end{example}
...
\begin{figure}
	...
	\caption{The ornamentation lattices~$\Orn(\Kgraph)$ and~$\Orn(\Tgraph)$. (Gray = cyclic). \andrew{Is it clear how the triangle is oriented?}}
...
\begin{definition}
\label{def:Reori2Orn}
For a reorientation~$R$ of~$\tc(D)$, we define a map~$\orn{R}$ on~$V$ which associates to each vertex~$v \in V$ the inclusion maximal ornament of~$D$ at~$v$ contained in the subset of vertices~$u \in V$ with a directed path to~$v$ in $\rev(R)$. \andrew{I think we should be a little more precise about what this means. I think somebody reading this could confuse $\rev(R)$ for either of the following: (i) the subgraph of only arrows in $\tc(D)$, but with all of the arrows reversed from their normal orientation. (ii) $\tc(D)$ but with the arrows in $\rev(R)$ reversed.}
\end{definition}
...
\medskip\noindent
To correct this default\Andrew{I think the word here is intended to be either ``fault'' or ``defect'', but I'm not sure which.}, we can restrict our attention to transitively closed reorientations of~$\tc(D)$.
\end{remark}
...
This surjection behaves much nicer with respect to \cref{rem:Orn2ReoriMiddle,rem:Reori2OrnIntervals1,rem:Reori2OrnSemilatticeMap}. \felix{when $D$ is a directed tree.}
...
\]
are \felix{\sout{both}} singletons, \felix{respectively} containing the reorientations
\[
...
The \defn{sourcing lattice}~$\Sour(\HH)$ is the lattice of sourcings of~$\HH$ ordered componentwise, meaning~$S_1 \le S_2$ if \felix{and only if}~$S_1(H) \le S_2(H)$ for all~$H \in \HH$.
...
\begin{remark}
Note that neither the map~$S \mapsto \reori{S}$, not the map~$S \mapsto \orn{S}$ are order-preserving.
\vincent{todo}
\end{remark}
...
We now recall the standard map from permutations to acyclic \felix{\sout{orientations} reorientations}.
...
For an acyclic reorientation~$R$ of~$\tc(D)$, we denote by~$\asour{R}$ the sourcing on~$\PP(D)$ where~$\asour{R}(P)$ is the source of~$P$ in~$R$ (it is well-defined since any path in~$D$ forms a clique in~$\tc(D)$). \Felix{Should we define the notion of sources for reorientations} \felix{For a path $P=[a_1,\dots,a_k]$ in $R$, do we mean that $R(P)=R((a_1,a_k))$? If $R((a_1,a_k))=a_k$, does that mean that $(a_1,a_k)\in \rev(R)$?}
\end{definition}
...
Since~$P$ forms a clique in~$\tc(D)$, and $R$ is acyclic, we conclude that~$(u,v) \in \rev(R)$. \felix{I don't see what is the nuance between the source of $P=[u,\dots,v]$ and $(u,v)\in \rev(R)$.}
\end{proof}
...
\vincent{I agree. I wanted to avoid geometry as much as possible. But we can at least include a remark.}
\end{remark}
