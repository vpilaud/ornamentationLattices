...
\item otherwise, we obtain a new induced alternating cycle by replacing~$v_k$ by~$w$ in~$C$. By induction on the sum of the length of the paths between~$u_i$ and~$v_i$ in~$T$, we obtain that~$k = 2$ and that there is~$w' \in V$ such that~$(u_1, w')$, $(u_2, w')$, $(w', v_1)$, and~$(w', w)$ are in~$\tc(T)$. As~$w$ is between~$u_1$ and~$v_2$, we also get~$(w', v_2) \in \tc(T)$.
\qedhere
\end{itemize}
