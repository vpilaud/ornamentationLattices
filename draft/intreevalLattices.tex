...
\vincent{I need to speak about transitively closed, coclosed, biclosed here}
...
\vincent{I need to add reorientation in this paragraph and statement...}
...
We use instead a natural quasi-lattice map from the ornamentation lattice~$\Orn(T)$ to the acyclic sourcing poset~$\ASour(\II)$ to obtain the following characterization.
...
\item \defn{transitively coclosed} if~$(u,v), (v,w) \notin \rev(R)$ implies $(u,w) \notin \rev(R)$,
...
There are some transitively biclosed reorientations of~$\tc(D)$ which are not acyclic, even when~$D$ is a tree.
For instance,
\[
...
\]
We will see that all transitively biclosed reorientations are acyclic when~$T$ is an unstarred tree.
\vincent{ref}
\end{remark}
...
Observe however that~$\Rbi(\tc(D))$ may fail to be a lattice when~$D$ is not a tree.
For instance, consider the following four transitively biclosed reorientations:
%\[
%\begin{array}{cccc}
...
We will see in \cref{prop:biclosedLatticeT} that~$\Rbi(\tc(T))$ is a lattice when~$T$ is a tree.
\end{remark}
...
\subsection{Closed, coclosed, biclosed reorientations}
...
If~$R$ is a biclosed and cyclic reorientation of~$\tc(T)$, then it covers at least two biclosed reorientations.
\end{lemma}
...
\vincent{I need to update this example...}
For instance,
\begin{itemize}
...
We will see further equivalent conditions in \cref{subsec:OrnUT}, see \cref{coro:starredCharacterizations} for a summary.
...
(ii) $\Rightarrow$ (iii). Let~$a,b,c,d,e$ as in~(ii). Let $u$ (resp.~$v$) be the first (resp. last) common vertex of the paths from~$a$ and~$b$ to~$c$ (resp. from~$c$ to~$d$ and~$e$). Then~$u$ (resp.~$v$) has at least two incoming (resp.~outgoing) arrows, and there is a path from~$u$ to~$v$ passing through~$c$.
...
For an unstarred tree~$T$, the acyclic reorientation lattice~$\AReori(\tc(T))$ is a lattice.
\end{proposition}
...
\begin{proposition}
For a starred tree~$T$, there exists a transitively biclosed and cyclic reorientation of~$\tc(T)$ and a cyclic reorientation.
...
\item the acyclic reorientation poset of~$\tc(T)$ is not a lattice,
\item the acyclic sourcing poset of~$\PP(T)$ is not a lattice,
\item the acyclic ornamentation poset of~$T$ is not a lattice.
\end{enumerate}
\end{corollary}
...
As the $(m,n)$-broom is a rooted tree, all its ornamentations are acyclic by \cref{prop:AOrnLatticeUT}, so its numbers of acyclic sourcings, acyclic ornamentations and ornamentations are all equal.
\end{remark}
...
\begin{conjecture}
The number of ornamentations of a comb is given by 1, 4, 26, 226, 2426, 30826, 451586, 7489426, 138722426, 2839238026, 63654973826, 1551919194226, 40888965122426, 1157981114051626, 35083865696279426, 1132449247218851026, 38800104353355372026, 1406432065083818193226, which is https://oeis.org/A105616.
\end{conjecture}
...
\vincent{Can we get the generating function?}
...
\begin{example}
Among the $7$ intreeval hypergraphs of \cref{fig:intreevalHypergraphs},
\begin{itemize}
\item the first two are both path intersection closed and path star sparse,
\item the next four are not path intersection closed but path star sparse,
\item the last one is path intersection closed but not path star sparse.
\end{itemize}

\begin{figure}
	\centerline{\includegraphics[scale=.8]{intreevalHypergraphs}}
	\caption{Some intreeval hypergraphs.}
...
\begin{example}
We illustrate the construction of the proof of \cref{lem:pathIntersectionClosed} the third intreeval hypergraph of \cref{fig:intreevalHypergraphs}.
\vincent{todo}
\end{example}
...
\begin{example}
We illustrate the construction of the proof of \cref{lem:pathStarSparse} the last intreeval hypergraph of \cref{fig:intreevalHypergraphs}.
\vincent{todo}
\end{example}
