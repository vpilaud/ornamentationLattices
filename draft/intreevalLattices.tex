...
\begin{figure}
	...
	\caption{The arrows in turquoise represent maps that are order preserving. From top to bottom, left to right, those results respectively follow from~\cref{lem:Sour2Reori,,lem:Reori2Orn2,,lem:Sour2Orn3,,lem:Orn2Reori1,,lem:Orn2Sour1,,lem:Orn2Sour1,,lem:AReori2ASour3}.  The red arrows represent maps that are not order preserving. Those results follow from definition of the map~$\areori{O}$ and from Remark~\ref{rem:ASour2AReori}.}
	\label{fig:allMaps}
\end{figure}

\begin{figure}
    \centerline{\input{figures/diagram1.tex}}
	\caption{These diagrams commute by \cref{lem:Sour2Orn1,,lem:Orn2Sour4,,lem:AReori2ASour2AOrn} respectively. Let us remark that when we restrict ourself to the acyclic case, the two top diagrams do not commute.}
\end{figure}

\begin{figure}
    \centerline{\input{figures/diagram1_new.tex}}
	\caption{These diagrams commute by \cref{lem:Sour2Orn1,,lem:Orn2Sour4,,lem:AReori2ASour2AOrn} respectively. Let us remark that when we restrict ourself to the acyclic case, the two top diagrams do not commute.}
\end{figure}
...
\felixantoine{What does cycles inside paths means?}
\Vincent{Bad formulation... I mean that the clique in $\tc(D)$ corresponding to a directed path does not necessarily have a source in an reorientation. Hence, I cannot define a sourcing because I need a source for each path.}
...
For instance, consider the directed graph~$D$, the two reorientations~$R_1$ and~$R_2$ of~$\tc(D)$, and the two sourcing~$S_1$ and~$S_2$ of~$S_1$ of~$\PP(D)$ given by
\begin{gather*}
D = \! \includegraphics[scale=.8,valign=c]{graphR}
\qquad\qquad
R_1 = \! \includegraphics[scale=.8,valign=c]{reorientationR1}
...
We conclude that~ \sout{$\orn{R}(v)$} \felix{$\rev(R)$}$ \subseteq \rev(\asour{R})$, hence that~$\orn{R}(v) \subseteq \orn{\asour{R}}(v)$ by maximality of~$\orn{\asour{R}}(v)$.
\end{proof}
...
\begin{proof}
\FelixAntoine{Since $O \mapsto \asour{O}$ is not the restriction of $O \mapsto \sour{O}$, we cannot use that the second is order-preserving to deduce that the first is. We modified the proof accordingly.}
\Vincent{Good point!}
...
%If~$O_1 \ne O$, then $O(v) \ssm O_1(v) \ne \varnothing$ and let~$y$ be a maximal element in $O(v) \ssm O_1(v)$.
\Vincent{I changed again as I feel my path is needed...}\Antoine{Yes, I am convinced that we need the path; this is the reason it works on trees and not on general graph. But we don't need $y$ and its maximality.}
If~$O_1 \ne O$, then there is~$x \in O(v) \ssm O_1(v)$.
% and let~$y$ be the last vertex not in~$O_1(v)$ along the path from~$x$ to~$v$ in~$O(v)$.
As~$x \in O(v) \ssm O_1(v) \subseteq O_2(v) \ssm  O_1(v) = O_1(u)$.
Hence, the directed path from~$x$ to~$v$ passes through~$u$, and we have~$u \in O(v)$.
...
\Antoine{\sout{No need the graph to be a tree. Should we put the result earlier in the article?}}
\Vincent{I disagree, the description of cover relations for arbitrary directed graphs is slightly more involved. You can find a counter-example in \cref{fig:ornamentationsKT}\,(left). At some point, I had the lemma below in \cref{subsec:ornamentations}. But I thought it was too complicated to have it at the beginning...}

\begin{lemma}
\label{lem:coverRelationsOrn}
Two ornamentations~$O_1$ and~$O_2$ of~$D$ form a cover relation~$O_1 \lessdot O_2$ if and only if there are~$u, v \in V$ such that~$u \notin O_1(v)$, $O_2(v) = O_1(u) \cup O_1(v)$ while~$O_1(w) = O_2(w)$ for all~${w \in V \ssm \{v\}}$, and~$O_1(w) = O_1(u)$ for any~$w \in O_1(u)$ with an edge towards~$O_1(v)$.
\Vincent{This statement will disappear or go to \cref{subsec:ornamentations}.}
\Antoine{I don't understand the last part. How could $O_1(w) = O_1(u)$ with $w<u$? Is it that $u$ is the only element in $O_1(u)\ssm O_1(v)$ that is covered by an element $x\in O_1(v)$?}
\end{lemma}
...
Fix~$u,v \in V$.
\sout{Observe first that~$u \in O(v)$ implies~$(u,v) \in \rev(\maxreori{O})$ since~$v \in O(w)$ implies~$u \in O(v) \subseteq O(w)$ for any~$w \in V$, so that~$u \in  \orn{\maxreori{O}}(v)$ by definition.} \felix{Observe first that~$u \in O(v)$ implies~$(u,v) \in \rev(\maxreori{O})$, since for any~$w \in V$, the fact that~$v \in O(w)$ implies~$u \in O(v) \subseteq O(w)$. Hence, by definition,~$u \in \orn{\maxreori{O}}(v)$.} \Felix{This is a reformulation}
Conversely, assume that~$u \notin O(v)$ and consider such an~$u$ which is \felix{the} closest to~$v$.
Consider the path~$u, u', \dots, v$ in~$T$.
For any~$v'$ between~$u'$ and~$v$, we have~\sout{$u \notin O(v)$} \felix{$u \notin O(v')$} while~$u', v' \in O(v)$, which implies that~$(u,v') \notin \rev(\maxreori{O})$.
...
Hence, $O(w) \cup \{u\}$ \Felix{Should we add that $\orn{R}(u)=\{u\}\in \orn{R}$} contradicts the maximality of~$\orn{R}(w)$.
...
Let~$w \in V$ be such that $u \notin O(w)$ but~$u',$\sout{$v'$} \felix{$v$} \Felix{Pointer for the typo in case we don't find it} $\in O(w)$.
...
Since any transitively closed reorientation~$R'$ such that~$R' > R_1$ and~$R' > R_2$ clearly satisfies~$R' > R$\Felix{$\geq$?}, we conclude that~$R$ is the join of~$R_1$ and~$R_2$ among transitively biclosed reoriorientations.
