...
%\pagebreak \Andrew{Do we need this pagebrak here? It's breaking the previous page.}
...
\item there are distinct $a,b,c,d,e \in V$ such that~$(a,c)$, $(b,c)$, $(c,d)$, and~$(c,e)$ are all in~$\tc(T)$, \Felix{We need to ask that $a$ and $b$ are not comparable in $T$ and that $d$ and $e$ are not comparable in $T$. Otherwise, a tree that is an interval can be starred accroding to this definition.}
...
The reorientation~$R$ of~$\tc(T)$ defined by~$\rev(R) = \set{(w,d)}{a \le_T w <_T d} \cup \set{(w,e)}{b \le_T w < e}$ is transitively closed because~$d \not<_T e$ and~$e \not<_T d$, transitively coclosed because~$(u,w) \in \rev(R)$ implies~$(v,w) \in \rev(R)$ for any~$u <_T v <_T w$, and cyclic because~\sout{$a, d, b, c$} \felix{a,e,b,d} forms a directed cycle.

The ornamentation~$O = O_R$ of~$T$ with~$O(c) = \set{w}{a \le_T w <_T d}$, $O(e) = \set{w}{b \le_T w < e}$, and~$O(w) = \{w\}$ for all~\sout{$w \in V \ssm \{d,e\}$} \felix{$w\in V \ssm \set{c}{a \le_T c <_T d \textit{ or } b \le_T c <_T e}$}  is cyclic.
...
We conclude that~~\sout{$a, d, b, c$} \felix{a,e,b,d} forms a directed cycle in~$R'$ as well.
Hence, $O$ is cyclic.
...
\item the \sout{ornamentation} \felix{reorientation} lattice~$\AReori(\tc(T))$ is (isomorphic to) the Tamari lattice of~$\tc(T)$ defined by the $\tc(T)$-sylvester congruence,
...
To fix a convention, let us denote by~$1, \dots, 2n-1$ the teeth and by~$2, \dots, 2n$ the handle of the $n$-comb, so that the parent of node~$i$ is $i+1$ if~$i$ is odd and~\sout{$i+n$} \felix{$i+2$} \Felix{Pointer} if~$i$ is even (except the node~$2n$ which is the root).
...
S_A < S_G
\qquad
\text{\sout{$S_B < S_D$} } \felix{S_B < S_G}
\qquad
S_G < S_C
\qquad
S_G < S_D.
...
\item $T^i(I) = S(I)$ except if~$\{u_i, v_i\} \subseteq I$ in which case we have~\sout{$T^i(I) = v_i < u_i = S(I)$} \felix{$S(I) = u_i < v_i = T^i(I)$}.
\end{itemize}
...
We also denote by~$w'_i$ the first vertex along the path joining~$w_i$ to~$w_{i+1}$ in~$T$.\Felix{This is not the definition of $w_i'$ that we want.}

As~\sout{$I_{i+2} \cup I_{i+3} \cup \dots \cup I_{i-2} \cup I_{i-1}$}\Felix{Not sure how I should interpret this union with the labelling of the $I_j$'s} \felix{$I_1 \cup I_2 \cup \cdots \cup I_{i-1} \cup I_{i+2} \cup I_{i+3} \cup  \cdots \cup I_{p-1} \cup I_p$ } is connected by~(a) and contains both~$w_{i-1}$ and~$w_{i+1}$ but not~$w_i$ by~(b), the (not necessarily directed) path joining~$w_{i-1}$ to~$w_{i+1}$ in~$T$ cannot contain~$w_i$.
