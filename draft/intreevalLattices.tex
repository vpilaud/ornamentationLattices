...
First, the Hasse diagram of the Tamari lattice is isomorphic to a linear orientation of the graph of the associahedron of~\cite{ShniderSternberg, Loday}, which is a Minkowski sum of faces of the standard simplex~\cite{Postnikov}.\Andrew{Does it make more sense to just say ``A Minkowski sum of standard simplices''?} 
...
Ornamentations were introduced and studied in~\cite{DefantSack} for rooted trees and extended in~\cite{Sack} for directed graphs and beyond. Ornamentations of rooted trees have been further studied in~\cite{ajran2025pop}. \Andrew{Added a citation for rooted tree ornamentations studied in a paper by Arjan and Defant. Feel free to remove if you feel necessary, but one result they prove in this paper is that the ornamentation lattice of a rooted tree is semidistributive.}
...
For an increasing tree~$T$ on~$[n]$, we call any subhypergraph~$\II$ of the path hypergraph~$\PP(T)$ an \defn{intreeval hypergraph} of~$T$, meaning any collection of (vertex sets of) directed paths in~$T$.
...
We clearly define \jose{different} ornamentations by
\begin{enumerate}[(i)]
