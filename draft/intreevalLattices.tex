...
The ornamentation~$O = \orn{R}$ of~$T$ with~$O(d) = \set{w}{a \le_T w \le_T d}$, $O(e) = \set{w}{b \le_T \le <_T e}$, and~$O(w) = \{w\}$ for all~$w \in V \ssm \{d,e\}$ is cyclic.
...
If \jose{$u <_T v$ but~}$u \notin \orn{R_1 \meet R_2}(v)$, then there is $u'$ along the path from~$u$ to~$v$ such that~$(u',v) \notin \rev(R_1 \meet R_2)$ by \cref{lem:Reori2OrnT}.
Therefore, the\jose{\sout{re is a}} path~$u' = w_0, w_1, \dots, w_k = v$ \jose{\sout{such that}satisfies}~$(w_{i-1}, w_i) \notin \rev(R_1) \cap \rev(R_2)$ \jose{for all $i \in [k]$} by~\cref{prop:AReoriLatJoinMeet}.
...
Since~$R_1$ is acyclic, \jose{\sout{we have}the minimality of $\ell$ implies that}~$(w_j, w_\ell) \in \rev(R_1)$ for all~${i \le j \le \ell}$.
